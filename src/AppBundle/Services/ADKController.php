...
class ADKController extends AdminController
{
    ...
    public function adkAction($numcampaigns, $timezone, $depdate)
    {
        ...
        ;
        ####selecting users that did not get email from us for 7 days + did not ever bounce hard
        ...
        $qb
            ...
            -> setParameters(['todaydate' => new DateTime()])
        ;

        $subscribers = $qb ->getQuery() ->getResult();

        # crunching big array of data onto arrays of 500 emails as per ADK requirement
